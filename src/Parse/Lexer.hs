...
uppercaseIdentifierRE =
  Text.append
    <$> psym (check isUpper) -- it actually check the Unicode!
    <*> (Text.concat <$> many (psym (check (\c -> isAlphaNum c || c == '_'))))

lowercaseIdentifierRE :: RE Text Text
lowercaseIdentifierRE =
  Text.append
    <$> psym (check isLower) -- it actually check the Unicode!
    <*> (Text.concat <$> many (psym (check (\c -> isAlphaNum c || c == '_'))))

intRE :: RE Text Int
intRE =
  read <$> (Text.unpack . Text.concat <$> (some . psym . check $ isDigit))
...
lexer = mconcat
  ...
  , whitespace
    (longestShortest (contra $ text "--")
                     (contra . (\_ -> many anySym *> text "\n"))
    )
  , whitespace
    (longestShortest (contra $ text "--[")
                     (contra . (\_ -> many anySym *> text "]--"))
